...
local sql
local load_errors = {}
local names = { "sqlite3", "libsqlite3.so.0", "libsqlite3.so", "sqlite3.dll" }
for _, name in ipairs(names) do
  local success, res = pcall(ffi.load, name)
  if success then
    sql = res
    break
  else
    table.insert(load_errors, name .. ": " .. tostring(res))
  end
end
if not sql then
  error("Unable to load sqlite3 library. Tried: " .. table.concat(load_errors, " | "))
end
